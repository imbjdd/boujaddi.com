...
export function getMarkerWidth(marker: LifelineMarker, nextYear?: number) {
  ...
  // A marker carrying a floating photo gets a wider slot so the card has
  // blank space to sit in beside the text, instead of landing on the
  // next year's column.
  if (hasContent && marker.photos?.length) return 640
